...
- A shared-mutable field (mutex-guarded cell) whose only callers lock it and mutate inline. → The shared-mutation case has no owner. Either define one, or move to message-passing via an event bus ([[architecture-canon]] `references/smalltalk.md`).
